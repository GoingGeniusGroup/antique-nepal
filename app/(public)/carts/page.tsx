...
const Cart = ({ userId }: { userId: string }) => {
  ...
  const removeItem = async () => {
    ...
    const previousData = data; // keep backup for rollback
    mutate(
      (currData) => ({
        items: currData!.items.filter((item) => item.id !== id),
      }),
      false // don't revalidate yet
    );
            ...
            {items.length === 0 ? (
              <div className="text-center py-20 bg-accent/30 rounded-lg border border-border">
                ...
                <p className="text-muted-foreground mb-8 max-w-md mx-auto">
                  Discover our collection of handcrafted hemp bags made by
                  skilled Nepali artisans
                </p>
