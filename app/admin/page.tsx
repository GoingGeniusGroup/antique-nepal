...
import { Users, Package, ArrowRight, ShoppingCart, DollarSign } from "lucide-react";
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const ordersRes = await fetch("/api/admin/orders?page=1&pageSize=5&sort=createdAt&order=desc");
    ...
    <PageTransition>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <Card className="p-6 dark:!bg-slate-800 dark:!border-slate-600">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-foreground dark:text-slate-100">Recent Orders</h2>
              <Link 
                href="/admin/orders"
                className="flex items-center gap-1 text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors"
              >
            ...
            <div className="space-y-3">
              {loading ? (
                ...
                recentOrders.map((order) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-foreground dark:text-slate-100 truncate">#{order.orderNumber}</p>
                      <p className="text-xs text-muted-foreground dark:text-slate-400 mt-0.5">{order.user.email}</p>
                    </div>
                    <div className="flex items-center gap-3 ml-4">
                      <div className="text-right">
                        <p className="text-sm font-bold text-foreground dark:text-slate-100">{formatCurrency(order.total)}</p>
                      ...
                      <p className="text-xs text-muted-foreground dark:text-slate-400 whitespace-nowrap">{formatDate(order.createdAt)}</p>
                    </div>
                  </div>
                ))
              )}
